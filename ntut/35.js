...
var tarparse = require('tar').Parse();
var zlib = require('zlib')
var bl = require('bl') // buffer list as alt to through
...
var decrypt = crypto.createDecipher(cipher, pass);
process.stdin.pipe(decrypt)
  .pipe(zlib.createGunzip())
//  .pipe(process.stdout);
  .pipe(tarparse)

// each entry in the tar as a stream
tarparse.on('entry', function (entry) {
  entries++;
//  console.log("Found entry nr " + entries)
//  console.dir(entry)
  if(entry.type !== "File") return;
//  if(entry.type === "Directory")
//	console.dir(entry.path + " is a Dir")
//  console.dir(entry.path + " is a File")

  entry.pipe(crypto.createHash('md5', { encoding: 'hex' }))
	.pipe(tr(null, end)).pipe(process.stdout)
// alt:
